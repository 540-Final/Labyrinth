...
module map (
	...
	input 		[8:0]	wrld_col_addr,	// column address of world map location
	...
	input 		[9:0]	vid_col,		// video logic column address
				[9:0]	vid_row,		// video logic row address
...
reg 	[16:0] 	wrld_mem_addr, vid_mem_addr;	// dual port RAM addresses for 
...
always @ (*) begin
	wrld_mem_addr <= {wrld_row_addr[7:0], wrld_col_addr[8:0]};
	
	if ( reset) begin
		vid_mem_addr <= 17'b0;
	end else begin
		if (vid_row[7:0] <= 240 && vid_col[8:0] <= 320)
			vid_mem_addr <= vid_row[7:0]*320 + vid_col[8:0];
		else
			vid_mem_addr <= 17'd76801;		
	end
