...
reg 	[16:0] 	a_mem_addr, b_mem_addr;	// dual port RAM addresses for 
...
always @ (*) begin	
	if ( reset) begin
		...
		if (a_row_addr[7:0] <= MAP_HEIGHT && a_col_addr[8:0] <= MAP_WIDTH) begin
			a_mem_addr <= a_row_addr[7:0]*MAP_WIDTH + a_col_addr[8:0]; 
			end
		else begin
			a_mem_addr <= `NULL_ADDR;
			end
				
        if (b_row_addr[7:0] <= MAP_HEIGHT && b_col_addr[8:0] <= MAP_WIDTH) begin
            b_mem_addr <= b_row_addr[7:0]*MAP_WIDTH + b_col_addr[8:0];
            end
        else begin
